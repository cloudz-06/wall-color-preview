...
(async () => {
  ...
  // Navigate via router, not page.goto to simulate real user behavior
  ...
  await page.evaluate(() => {
    ...
    try {
        ...
        console.log('BROWSER: Quota Error during setup:', e.message);
    }
  });
  ...
  await page.evaluate((id) => {
    const editBtn = document.getElementById('edit-variation-' + id);
    if(editBtn) editBtn.click(); // This will trigger navigate('/editor') without reload
    else {
        window.useEditorStore.getState().loadVariation(id);
  ...
  await page.evaluate(() => {
    const s = window.useEditorStore.getState();
    s.updateWall(s.walls[0].id, { color: '#ff00ff' });
  });
  await page.waitForTimeout(500);
